module AdminAuth
  module Sessions
    class RotateSession
      Result = Struct.new(:success?, :admin, :access_token, :refresh_token, keyword_init: true)

      class << self
        def call(refresh_token:)
          decoded = AdminAuth::Jwt::Verifier.verify!(token: refresh_token, expected_type: "refresh")
          refresh_jti = decoded.payload.fetch("jti")
          admin_id = decoded.payload.fetch("sub")
          ...
          session.with_lock do
            ...
            return Result.new(success?: false) unless session.admin.active?

            access = AdminAuth::Jwt::Issuer.issue_access(admin_id: session.admin_id)
            new_refresh = AdminAuth::Jwt::Issuer.issue_refresh(admin_id: session.admin_id)
            now = Time.current
            ...
            Result.new(success?: true, admin: session.admin, access_token: access, refresh_token: new_refresh)
          end
