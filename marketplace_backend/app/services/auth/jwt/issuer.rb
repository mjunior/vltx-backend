...
module Auth
  module Jwt
    class Issuer
      ...
      class << self
        def issue_access(user_id:, now: Time.current)
          issue(type: "access", user_id: user_id, now: now)
        end

        def issue_refresh(user_id:, now: Time.current)
          issue(type: "refresh", user_id: user_id, now: now)
        end

        private

        def issue(type:, user_id:, now:)
